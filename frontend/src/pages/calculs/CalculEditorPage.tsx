...
import { cn, calculStatusLabels, calculStatusColors, typeProduitLabels } from '@/lib/utils'
import type { CalculParametres, NormeType } from '@/types'
...
import { NormeBadge } from '@/components/common'
import PdfPlanUploader, { type ExtractedParameters } from '@/components/calculs/PdfPlanUploader'
...
export default function CalculEditorPage() {
  ...
  useEffect(() => {
    ...
    } else if (calcul?.type_produit?.toLowerCase() === 'plancher_poutrelles_hourdis') {
  ...
  const handleApplyPdfData = (extracted: ExtractedParameters) => {
    setParametres((prev) => ({
      ...prev,
      geometrie: {
        ...prev.geometrie,
        portee: extracted.portee_m ?? prev.geometrie.portee,
      },
      charges: {
        ...prev.charges,
        permanentes: extracted.charge_permanente_kn_m2 ?? prev.charges.permanentes,
        exploitation: extracted.charge_exploitation_kn_m2 ?? prev.charges.exploitation,
      },
    }))
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <span>{typeProduitLabels[calcul.type_produit]}</span>
              <span>-</span>
        ...
        <div className="flex items-center gap-3">
          <span
            className={cn(
              ...
              calculStatusColors[calcul.status]
            )}
          >
            {calculStatusLabels[calcul.status]}
          </span>
          {isComputed && (
            <ExportButtons
              calculId={id}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {showPdfUpload && (
              <div className="px-6 pb-6 border-t">
                <p className="text-sm text-gray-500 mt-4 mb-4">
                  Uploadez un plan de coffrage ou une note de calcul pour extraire
                  automatiquement les paramètres (portées, charges, etc.)
                </p>
                <PdfPlanUploader onApplyData={handleApplyPdfData} />
              </div>
            )}
          </div>

          {/* Formulaire spécifique selon le type de calcul */}
          {calcul.type_produit?.toLowerCase() === 'plancher_poutrelles_hourdis' ? (
            <PlancherPoutrellesForm
        ...
        {/* Results */}
        <div className="space-y-6">
          {/* Affichage des résultats selon le type de calcul */}
          {calcul.type_produit?.toLowerCase() === 'plancher_poutrelles_hourdis' ? (
            <PlancherResultats calcul={calcul} />
          ) : (
            <>
              {/* Summary */}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        ...
                        {resultats.summary.norme_utilisee && (
                          <NormeBadge norme={resultats.summary.norme_utilisee as NormeType} size="sm" />
                        )}
                      </div>
              ...
              {/* Not computed yet */}
              {!isComputed && calcul.status !== 'computing' && calcul.status !== 'error' && (
